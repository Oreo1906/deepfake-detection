"""
Skin Model - Detects skin texture and frequency artifacts in deepfake images
Triple-stream architecture with 4 heads: classification + texture + artifact + frequency
"""
...
class SkinModel(nn.Module):
    """
    Triple-stream + 4 heads: classifier + texture + artifact + frequency
    
    Input: Three image tensors (B, 3, 224, 224) each:
        - rgb: Normal RGB image
        - hf: High-frequency component
        - lap: Laplacian filter output
    
    Output: Dict with keys:
        - "logits": (B, 2) - Real/Fake classification logits
        - "texture": (B,) - Skin texture anomaly score [0-1]
        - "artifact": (B,) - Skin artifact score [0-1]
        - "frequency": (B,) - Frequency domain anomaly score [0-1]
    """
